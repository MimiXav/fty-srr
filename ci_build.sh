...
default|default-Werror|default-with-docs|valgrind|clang-format-check)
    ...
    ; then
        echo ""
        BASE_PWD=${PWD}
        echo "`date`: INFO: Building prerequisite 'augeas' from Git repository..." >&2
        cd ./tmp-deps
        ...
        cd "${BASE_PWD}"
        CONFIG_OPTS+=("--with-augeas=yes")
    else
        CONFIG_OPTS+=("--with-augeas=yes")
    fi

    # Start of recipe for dependency: libxml-2.0
    if ! (command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list libxml-2.0-dev >/dev/null 2>&1) || \
           (command -v brew >/dev/null 2>&1 && brew ls --versions libxml-2.0 >/dev/null 2>&1) \
    ; then
        echo ""
        echo "WARNING: Can not build prerequisite 'libxml-2.0'" >&2
